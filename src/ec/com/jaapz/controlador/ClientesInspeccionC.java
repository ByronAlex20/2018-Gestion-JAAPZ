...
public class ClientesInspeccionC {
	...
	@FXML Tab tpNuevas;
	@FXML TableView<Inspeccion> tvAsignados;
	@FXML TableView<Inspeccion> tvNuevosAsig;
	...
	void initialize() {
		...
		bloquearIns();
		llenarCombosIns();
		cboEstadoIns.setText("PENDIENTE");;
		...
		try {
			if(tvDatosClientes.getSelectionModel().getSelectedItem() != null) {
				Optional<ButtonType> result = helper.mostrarAlertaConfirmacion("Se procederá a dar de baja al cliente.."
						+ "\nNombre: "
						+ tvDatosClientes.getSelectionModel().getSelectedItem().getNombres() + " " + tvDatosClientes.getSelectionModel().getSelectedItem().getApellidos() 
						+ "\nCédula: "
	...
	void imprimirListado(ActionEvent event) {
		try {
			...
			printReport.crearReporte("/src/ec/com/jaapz/informes/lista_cliente.jasper", clienteDAO, param);
	...
	void llenarDatosIns(Cliente datosSeleccionado) {
		try {
			...
			txtNombresIns.setText(datosSeleccionado.getNombres());
			txtApellidosIns.setText(datosSeleccionado.getApellidos());
	...
	private boolean grabarDatos() {
		try {
			...
			if(result.get() == ButtonType.OK){
				...
				inspeccion.setIdInspeccion(null);
	...
	public void buscarOrdenEmitida() {
		try {
			...
			if (Context.getInstance().getInspeccion() != null) {
				Inspeccion ordenAgregar = Context.getInstance().getInspeccion();
	...
	@SuppressWarnings("unchecked")
	private void agregarOrden(Inspeccion ordenAgregar) {
		try {
	...
	public void quitarAsignacion() {
		try {
			Inspeccion ordenQuitar = tvNuevosAsig.getSelectionModel().getSelectedItem();
			if(ordenQuitar == null) {
	...
	public void grabarAsig() {
		try {
			...
			if(result.get() == ButtonType.OK){
				inspeccionDAO.getEntityManager().getTransaction().begin();
				for(Inspeccion inspeccionGrabar : tvNuevosAsig.getItems()) {
	...
	private void recuperarPersonalInspeccion(SegUsuario personalInspeccion) {
		try {
			List<Inspeccion> listadoInspecciones = inspeccionDAO.getListaInspeccionAsignada(personalInspeccion.getIdUsuario());
	 ...
	 *operaciones de la vista de inspeeciones realizadas
	 +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
	@FXML private TableView<Inspeccion> tvDatosInspecciones;
	...
	private void recuperarInspeccionesRealizadas(String criterio) {
		try{
			...
			//llenar los datos en la tabla
			TableColumn<Inspeccion, String> idColum = new TableColumn<>("Cédula");
			idColum.setMinWidth(10);
			idColum.setPrefWidth(90);
			idColum.setCellValueFactory(new Callback<TableColumn.CellDataFeatures<Inspeccion,String>, ObservableValue<String>>() {
				@Override
				public ObservableValue<String> call(CellDataFeatures<Inspeccion, String> param) {
			...
			TableColumn<Inspeccion, String> estadoColum = new TableColumn<>("Est. de Inspección");
			estadoColum.setMinWidth(10);
		...
		try {
			if(tvDatosInspecciones.getSelectionModel().getSelectedItem() != null) {
				...
				Optional<ButtonType> result = helper.mostrarAlertaConfirmacion("Se procederá a dar de baja a la Inspección generada al Cliente.."
						+ "\nNombre: "
						+ tvDatosInspecciones.getSelectionModel().getSelectedItem().getCliente().getNombres() + " " + tvDatosInspecciones.getSelectionModel().getSelectedItem().getCliente().getApellidos() 
						+ "\nCédula: "
						+ tvDatosInspecciones.getSelectionModel().getSelectedItem().getCliente().getCedula()
						+ " Fecha: "
						+ tvDatosInspecciones.getSelectionModel().getSelectedItem().getFecha()
						+ "\n\nDesea Continuar?",Context.getInstance().getStage());
				if(result.get() == ButtonType.OK){
	...
	public void imprimirInspeccion() {
		try {
			if(tvDatosInspecciones.getSelectionModel().getSelectedItem() != null) {
				...
				Inspeccion ins = tvDatosInspecciones.getSelectionModel().getSelectedItem();
				Map<String, Object> param = new HashMap<String, Object>();
				param.put("ID_CLIENTE", ins.getCliente().getIdCliente());
				param.put("referencia", ins.getReferencia());
				
				if(ins.getUsoMedidor().equals("VIVIENDA"))
					param.put("vivienda", "X");
				else
					param.put("vivienda", "");
				
				if(ins.getUsoMedidor().equals("COMERCIAL"))
					param.put("comercial", "X");
				else
					param.put("comercial", "");
				
				if(ins.getUsoMedidor().equals("ESTABLECIMIENTO PÚBLICO"))
					param.put("publico", "X");
				else
					param.put("publico", "");
				
				
				param.put("fecha_inspeccion", formateador.format(ins.getFecha()));
				...
				PrintReport printReport = new PrintReport();
				printReport.crearReporte("/src/ec/com/jaapz/informes/ficha_inspeccion.jasper", clienteDAO, param);
