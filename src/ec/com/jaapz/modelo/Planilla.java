...
@NamedQueries({
	@NamedQuery(name="Planilla.findAll", query="SELECT p FROM Planilla p "
			+ "where (lower(p.cuentaCliente.cliente.apellidos) like :patron or lower(p.cuentaCliente.cliente.nombres) like :patron or lower(p.cuentaCliente.cliente.cedula) like :patron) and p.cancelado = 'N' "
